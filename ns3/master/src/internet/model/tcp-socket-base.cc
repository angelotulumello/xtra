...
//                   MakeEnumChecker (CLOSED, "Closed"))
    .AddAttribute ("Verbose",
                   ...
                   UintegerValue (1),
                   MakeUintegerAccessor (&TcpSocketBase::m_verbose),
      ...
      if ((m_state == ESTABLISHED || m_state == CLOSE_WAIT) && AvailableWindow () > 0)
        { // Try to send the data out: Add a little step to allow the application
          ...
          if (!m_sendPendingDataEvent.IsRunning ())
            {
              // only the first time... then leave it to xfsm
              if (xfsm_configured && m_tcb->m_nextTxSequence == (SequenceNumber32) 1)
...
if(ackNumber.GetValue() > 1 && (m_state==ESTABLISHED || m_state==CLOSE_WAIT) && xfsm_configured) {
  ...
  {
    if (m_verbose) fprintf(stderr, "===> It's a SACK!\n");
    uint32_t left[] = {0, 0, 0}; 
    ...
    for (option_it = list.begin (); option_it != list.end (); ++option_it, i++)
    {
      ...
      if (m_verbose) fprintf(stderr, "SACK is from: %d to: %d\n", left[i], right[i]);
    }
  ...
  if (m_verbose) fprintf(stderr, "------ tableLookup()\n");
  TableEntry *match = xfsm->tableLookup();

  if (match != NULL){
    if (m_verbose) fprintf(stderr, "------ found match; executeActions()\n");
    executeActions(*match);
    if (m_verbose) fprintf(stderr, "------ state transition to: %u\n", match->getEntryNextState());
    xfsm->current_state = match->getEntryNextState();
    if (m_verbose) fprintf(stderr, "------ sync registers()\n");
    syncRegisters();
  } 
  else 
    if (m_verbose) fprintf(stderr, "------ match not found: doing nothing\n");
...
void TcpSocketBase::OnGenericTimerReceived(uint32_t param)
{
  ...
  if (m_verbose) fprintf(stderr, "AvailableWindow is %d\n", xfsm->registers[XFSM_AVAILABLE_WIN] );
  TableEntry *match = xfsm->tableLookup();
  if (match != NULL){
    if (m_verbose) fprintf(stderr, "------ found match; executeActions()\n");
    executeActions(*match);
    if (m_verbose) fprintf(stderr, "------ state trnasition to: %u\n", match->getEntryNextState());
    xfsm->current_state = match->getEntryNextState();
    if (m_verbose) fprintf(stderr, "------ sync registers()\n");
    syncRegisters();
  } else 
...
TcpSocketBase::SendBuffer (bool withAck)
{
  ...
  if (m_verbose) fprintf(stderr, "Available window is: %u\n", availableWindow);
...
void TcpSocketBase::initializeRegisters()
{
  ...
  xfsm->registers[XFSM_SSTHRESH] = 0;
  ...
  xfsm->registers[XFSM_RCVTIMESTAMPVALUEECHOREPLY] = m_tcb->m_rcvTimestampEchoReply;
  xfsm->registers[XFSM_RTT] = (uint32_t) 0;
  ...
  xfsm->registers[XFSM_CUSTOM5] = (uint32_t) 1;
  xfsm->registers[XFSM_CUSTOM6] = (uint32_t) 0;
  xfsm->registers[XFSM_CUSTOM7] = (uint32_t) 0;
}
...
void TcpSocketBase::syncRegisters()
{
  ...
  if(m_tcb->m_cWnd != xfsm->getRegister(Registers_t::XFSM_CWND)) { // only upload if it's changed,
    ...
    if (m_verbose) fprintf(stderr, "Updating cWnd to: %u\n", xfsm->getRegister(Registers_t::XFSM_CWND));
  }
  ...
  if(m_tcb->m_nextTxSequence.Get().GetValue() != xfsm->getRegister(Registers_t::XFSM_NEXTTXSEQ))
    m_tcb->m_nextTxSequence = (SequenceNumber32) xfsm->getRegister(Registers_t::XFSM_NEXTTXSEQ);
...
void TcpSocketBase::syncXfsmRegisters()
{
  ...
  xfsm->registers[XFSM_TIMESTAMP] = TcpOptionTS::NowToTsValue ();;

  if (m_verbose) fprintf(stderr, "timestamp=%u\n", xfsm->registers[XFSM_TIMESTAMP]);
}
...
void TcpSocketBase::executeActions(TableEntry &match)
{
  ...
  for (uint i = 0; i < match.getActions().size(); ++i){
      if(m_verbose) fprintf(stderr, "XfsmAction %u, action_type %d\n", i, (int)actions[i].getActionType());

    switch (actions[i].getActionType()){
        case XFSM_SENDBUFF:
        {
            if (m_verbose) fprintf(stderr, "*** Executing sendbuff\n");
            syncRegisters();
        ...
        case XFSM_SENDPCKT:
        {
            uint32_t sz = SendDataPacket((SequenceNumber32) xfsm->registers[XFSM_ARG0], xfsm->registers[XFSM_ARG1], true); 
            if (m_verbose) fprintf(stderr, "Executing send packet with seq: %d and size %d\n", xfsm->registers[XFSM_ARG0], sz);
            break;
        }
        case XFSM_UPDATE:
        {
            xfsm->registers[actions[i].output] = actions[i].update(xfsm->registers);
            if (m_verbose) fprintf(stderr, "*** Executing Update, result: %d\n", xfsm->registers[(int) actions[i].output]);
            break;
        }
        case XFSM_SETTIMER: //ARG0:: seqNumber  ARG1:: retrNumber  ARG2:: timeout
        {
            ...
            if (m_verbose) fprintf(stderr, "*** Executing setTimer at time %f, scheduled at time %f\n", 
                            Simulator::Now().GetSeconds(),
                            Simulator::Now().GetSeconds() + Time((xfsm->registers[XFSM_ARG2])*1000).GetSeconds());
        ...
        case XFSM_SETGENERICTIMER:  // ARGO:: timeout   //arg1:: param
        {
            ...
            if (m_verbose) fprintf(stderr, "*** Executing setGenericTimer at time %f, scheduled at time %f\n", 
                            Simulator::Now().GetSeconds(),
                            Simulator::Now().GetSeconds() + schedTime.GetSeconds());
            Simulator::Schedule(schedTime, &TcpSocketBase::OnGenericTimerReceived, this, 
                                xfsm->registers[XFSM_ARG1]);
            break;
        }
        ...
        case XFSM_REMOVETIMERS: // arg0: from, arg1 to
        {
          xfsm->removeTimers(xfsm->registers[XFSM_ARG0], xfsm->registers[XFSM_ARG1]);
          break;
        }
        default:
...
TcpSocketBase::SendDataPacket (SequenceNumber32 seq, uint32_t maxSize, bool withAck)
{
  ...
  if (sz == 0){
    p = Create<Packet> (maxSize);
    //fprintf(stderr, "\nCreated packet at %f with pktNo=%u", Simulator::Now().GetSeconds(), seq.GetValue()/maxSize);
  } 
  ...
  if (m_endPoint)
    {
      m_tcp->SendPacket (p, header, m_endPoint->GetLocalAddress (),
                         m_endPoint->GetPeerAddress (), m_boundnetdevice);
      NS_LOG_LOGIC(Simulator::Now().GetSeconds() << "Send segment of size " << sz << " with remaining data " <<
...
TcpSocketBase::BytesInFlight () const
{
  ...
  if (m_verbose) fprintf(stderr, "There are %u bytes in flight\n", bytesInFlight);
