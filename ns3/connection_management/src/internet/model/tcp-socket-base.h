...
public:
  ...
  void SetXfsm( TcpXfsm * xfsm) override;
    /**
...
protected:
  ...
  void syncRegisters();
  void executeActions(TableEntry &match);
  void initializeRegisters();
  void syncXfsmRegisters();
  void OnTimerReceived(uint32_t seqNumber);
  uint32_t SendBuffer (bool withAck);
  void OnGenericTimerReceived(uint32_t param);
  bool IsASack(const TcpHeader& tcpHeader);
...
protected:
  ...
  Time              m_minRto;          //!< minimum value of the Retrsmit timeout
