...
typedef enum {
    XFSM_FLAG_SYN = TcpHeader::SYN,
    XFSM_FLAG_ACK =TcpHeader::ACK,
    XFSM_FLAG_FIN = TcpHeader::FIN,
    XFSM_FLAG_RST = TcpHeader::RST
} Flags_t;

// for now, we declare our states for establishing and closing connections here, but this is not
// where they're supposed to be,
// in fact, they're just used for creating table entries in the examples
typedef enum {
    XFSM_CLOSED= 300,
    XFSM_LISTEN,
    XFSM_SYNSENT,
...
public:
    ...
    void setTcpFlags(TcpHeader tcpHeader);
