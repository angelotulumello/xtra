...
void TcpSocketBase::SetXfsm(TcpXfsm * externXfsm) 
{
  xfsm = externXfsm;
  initializeRegisters();
  xfsm_configured = true;
}
  ...
  if (m_txBuffer->SizeFromSequence (m_tcb->m_nextTxSequence) > 0)
    { // App close with pending data must wait until all data transmitted
      if (m_closeOnEmpty == false)
        ...
        // Here, if we want to close our xfsm, we must continue and call DoClose()
        if (xfsm_configured)
          DoClose();
      return 0;
    }
...
TcpSocketBase::DoConnect (void)
{
  ...
  if (xfsm_configured) {
    fprintf(stderr, "Hey I am the one who calls the syncXfsmRegisters!\n");
    syncXfsmRegisters();
    xfsm->xfsmWakeup(XFSM_CONNECT, 0, 0); //nothing to pass as parameters in this case
  
    fprintf(stderr, "------ tableLookup() in DoConnect()\n");
    TableEntry *match = xfsm->tableLookup();

    if (match != NULL){
      fprintf(stderr, "------ found match; executeActions()\n");
      executeActions(*match);
      fprintf(stderr, "------ state transition to: %u\n", match->getEntryNextState());
      xfsm->current_state = match->getEntryNextState();
      fprintf(stderr, "------ sync registers()\n");
      syncRegisters();
    } 
    //adding this manually, otherwise xfsm wouldn't work
    m_state = SYN_SENT;
  }
...
TcpSocketBase::DoClose (void)
{
  ...
  switch (m_state)
    ...
    case ESTABLISHED:
      if (xfsm_configured) {
        syncXfsmRegisters();
        xfsm->xfsmWakeup(XFSM_CLOSE, 0, 0); //nothing to pass as parameters in this case
        fprintf(stderr, "------ tableLookup()\n");
        TableEntry *match = xfsm->tableLookup();
    ...
    case LAST_ACK:
      ...
      fprintf(stderr, "LAST_ACK OR LISTEN\n");      
      CloseAndNotify ();
      break;
    case CLOSED:
    case FIN_WAIT_1:
...
bool TcpSocketBase::IsASack(const TcpHeader& tcpHeader)
{
  TcpHeader::TcpOptionList::const_iterator it;
  const TcpHeader::TcpOptionList options = tcpHeader.GetOptionList ();
...
if(ackNumber.GetValue() > 1 && m_state==ESTABLISHED && xfsm_configured) {
  fprintf(stderr, "\nNew ACK recived: #%d\n", ackNumber.GetValue());

  NewAck(ackNumber, true);

  fprintf(stderr, "\n------ Entering xfsm stage\n");
  xfsm->setTcpFlags(tcpHeader);
  syncXfsmRegisters();

  if(IsASack(tcpHeader)) // ack received contains sack blocks
  {
    uint32_t left[] = {0, 0, 0}; 
    uint32_t right[] = {0, 0, 0};

    TcpHeader::TcpOptionList::const_iterator it;
    const TcpHeader::TcpOptionList options = tcpHeader.GetOptionList ();

    Ptr<const TcpOption> option;

    for (it = options.begin (); it != options.end (); ++it)
    {
      option = (*it);
      // Check only for ACK options here
      if(option->GetKind () == TcpOption::SACK)
      {
        break;
      }
    }

    NS_ASSERT (option->GetKind () == TcpOption::SACK);
    ...
    int i = 0;
    for (option_it = list.begin (); option_it != list.end (); ++option_it, i++)
    {
      const TcpOptionSack::SackBlock b = (*option_it);

      left[i] = b.first.GetValue();
      right[i] = b.second.GetValue();
    }
    
    xfsm->setSackBlocks(left, right);
    xfsm->xfsmWakeup(XFSM_SACK, ackNumber.GetValue(), 0);
  }
  else 
  {
    Ptr<const TcpOptionTS> ts;
    ts = DynamicCast<const TcpOptionTS> (tcpHeader.GetOption (TcpOption::TS));
    Time m = TcpOptionTS::ElapsedTimeFromTsValue (ts->GetEcho ());
    uint32_t packetRTT = (uint32_t) m.GetMicroSeconds();
    uint32_t packetACK = (uint32_t) ackNumber.GetValue();
    xfsm->xfsmWakeup(XFSM_ACK, packetACK, packetRTT);
  }
  ...
  if (match != NULL){
    ...
    fprintf(stderr, "------ match not found: doing nothing\n");
...
/* Calling this method passing as an argument the xfsm, it will sync the value
 * of the xfsm's registers with the tcb and socket attributes.
 */
void TcpSocketBase::OnGenericTimerReceived(uint32_t param)
{
  fprintf(stderr, "\n%f OnGenericTimerReceived\n\n", Simulator::Now().GetSeconds());
  syncXfsmRegisters();
  xfsm->xfsmWakeup(XFSM_GENERIC_TIMEOUT, param, 0);
  fprintf(stderr, "------ tableLookup()\n");
  fprintf(stderr, "AvailableWindow is %d\n", xfsm->registers[XFSM_AVAILABLE_WIN] );
  TableEntry *match = xfsm->tableLookup();
  fprintf(stderr, ">>>>>> current state is:%u\n", xfsm->current_state);
  if (match != NULL){
    fprintf(stderr, "------ found match; executeActions()\n");
    executeActions(*match);
    fprintf(stderr, "------ state trnasition to: %u\n", match->getEntryNextState());
    xfsm->current_state = match->getEntryNextState();
    fprintf(stderr, "------ sync registers()\n");
    syncRegisters();
  } else 
...
void TcpSocketBase::OnTimerReceived(uint32_t seqNumber)
{
  fprintf(stderr, "\n%f OnTimerReceived, seq: %d, retxCount: %d\n\n", Simulator::Now().GetSeconds(),
                                                        xfsm->registers[XFSM_CONTEXT_PARAM0],
                                                        xfsm->registers[XFSM_CONTEXT_PARAM1]);
  syncXfsmRegisters();

  xfsm->xfsmWakeup(XFSM_TIMEOUT, seqNumber, 0);
  if (xfsm->registers[XFSM_CONTEXT_PARAM0] == 0) //the timer was not found, exit without executing any action
    return; // This can happen when a timer is removed before it fires!
  fprintf(stderr, "------ tableLookup()\n");
  TableEntry *match = xfsm->tableLookup();
  fprintf(stderr, ">>>>>> current state is:%u\n", xfsm->current_state);
...
uint32_t
TcpSocketBase::SendBuffer (bool withAck)
{
  ...
  fprintf(stderr, "Available window is: %u\n", availableWindow);
...
void TcpSocketBase::initializeRegisters()
{
  xfsm->registers[XFSM_CWND] = m_tcb->m_initialCWnd*m_tcb->m_segmentSize;
  xfsm->registers[XFSM_SSTHRESH] = m_tcb->m_ssThresh;
  xfsm->registers[XFSM_SEGMENTSIZE] = m_tcb->m_segmentSize;
  xfsm->registers[XFSM_LASTACKEDSEQ] = m_tcb->m_lastAckedSeq.GetValue();
  xfsm->registers[XFSM_HIGHTXMARK] = m_tcb->m_highTxMark.Get().GetValue();
  xfsm->registers[XFSM_NEXTTXSEQ] = m_tcb->m_nextTxSequence.Get().GetValue();
  xfsm->registers[XFSM_RCVTIMESTAMPVALUE] = m_tcb->m_rcvTimestampValue;
  xfsm->registers[XFSM_RCVTIMESTAMPVALUEECHOREPLY] = m_tcb->m_rcvTimestampValue;
  xfsm->registers[XFSM_RTT] = (uint32_t) m_rtt->GetEstimate().GetMicroSeconds();
  xfsm->registers[XFSM_SOCKETISCLOSED] = 0;
  xfsm->registers[XFSM_CUSTOM0] = (uint32_t) 0;
  xfsm->registers[XFSM_CUSTOM1] = (uint32_t) 0;
  xfsm->registers[XFSM_CUSTOM2] = (uint32_t) 0;
  xfsm->registers[XFSM_CUSTOM3] = (uint32_t) 0;
  xfsm->registers[XFSM_CUSTOM4] = (uint32_t) 0;
  xfsm->registers[XFSM_CUSTOM5] = (uint32_t) 0;
}

/* Calling this method, it will sync the value of the tcb attributes with
 * the values of the xfsm's registers.
 * xfsm -----> tcb
*/
void TcpSocketBase::syncRegisters()
{
  // Only few TCB registers need to be modified/updated (e.g. update last_acked_seq is nonsense)
  if(m_tcb->m_cWnd != xfsm->getRegister(Registers_t::XFSM_CWND)) { // only upload if it's changed,
    m_tcb->m_cWnd = xfsm->getRegister(Registers_t::XFSM_CWND); // because it's a traced value
    fprintf(stderr, "Updating cWnd to: %u\n", xfsm->getRegister(Registers_t::XFSM_CWND));
  }

  if(m_tcb->m_ssThresh != xfsm->getRegister(Registers_t::XFSM_SSTHRESH))
    m_tcb->m_ssThresh = xfsm->getRegister(Registers_t::XFSM_SSTHRESH);

  // maybe also updating the segment size is nonsense, by now we leave it as it is
  if(m_tcb->m_segmentSize != xfsm->getRegister(Registers_t::XFSM_SEGMENTSIZE))
    m_tcb->m_segmentSize = xfsm->getRegister(Registers_t::XFSM_SEGMENTSIZE);
}

/* Calling this method, it will sync the value of the xfsm registers with
 * the values of the tcb attributes.
 * tcb -----> xfsm
 */
void TcpSocketBase::syncXfsmRegisters()
{
  fprintf(stderr, "ENTERING SYNCXFSMREGISTERS\n, value is:%d\n", m_tcb->m_lastAckedSeq.GetValue());
  fprintf(stderr, "value is %p\t %p\n", xfsm->registers, &xfsm->registers[XFSM_LASTACKEDSEQ]);
  fprintf(stderr, ">>>>>> current state is:%u\n", xfsm->current_state);
  ...
  if(m_state == ESTABLISHED){
    ...
    xfsm->registers[XFSM_RCVTIMESTAMPVALUEECHOREPLY] = m_tcb->m_rcvTimestampValue;
    xfsm->registers[XFSM_RTT] = (uint32_t) m_rtt->GetEstimate().GetMicroSeconds();
    xfsm->registers[XFSM_AVAILABLE_WIN] = (uint32_t) AvailableWindow();
    xfsm->registers[XFSM_BYTESTOTRANSMIT] = (uint32_t) m_txBuffer->SizeFromSequence (m_tcb->m_nextTxSequence + m_tcb->m_segmentSize);
  }

}


void TcpSocketBase::executeActions(TableEntry &match)
{
  std::vector<XfsmAction> actions;
  actions = match.getActions();
  for (uint i = 0; i < match.getActions().size(); ++i){
      fprintf(stderr, "XfsmAction %u, action_type %d\n", i, (int)actions[i].getActionType());

    switch (actions[i].getActionType()){
        case XFSM_SENDBUFF:
        {
            fprintf(stderr, "*** Executing sendbuff\n");
            syncRegisters();
            SendBuffer(true); // SendPendingData wants a bool (with ack)... what we have to pass?
            break;
        }
        case XFSM_SENDPCKT:
        {
            fprintf(stderr, "Executing send packet with seq: %d\n", xfsm->registers[XFSM_ARG0]);
            uint32_t sz = SendDataPacket((SequenceNumber32) xfsm->registers[XFSM_ARG0], xfsm->registers[XFSM_ARG1], true); // what packet?
            m_tcb->m_nextTxSequence = (SequenceNumber32) xfsm->registers[XFSM_ARG0] + sz;
            break;
        }
        case XFSM_UPDATE:
        {
            xfsm->registers[actions[i].output] = actions[i].update(xfsm->registers);
            fprintf(stderr, "*** Executing Update, result: %d\n", xfsm->registers[(int) actions[i].output]);
            break;
        }
        case XFSM_SETTIMER: //ARG0:: seqNumber  ARG1:: retrNumber  ARG2:: timeout
        {
            fprintf(stderr, "SEQNUM TIMER ISSSSSSSSSSS %d\n", xfsm->registers[XFSM_ARG0]);
            xfsm->setTimer(xfsm->registers[XFSM_ARG0], xfsm->registers[XFSM_ARG1], xfsm->registers[XFSM_ARG2]);
            fprintf(stderr, "*** Executing setTimer at time %f, scheduled at time %f with seqnum = %d\n", 
                            Simulator::Now().GetSeconds(),
                            Simulator::Now().GetSeconds() + Time(xfsm->registers[XFSM_ARG2]*1000).GetSeconds(), xfsm->registers[XFSM_ARG0]);
            Simulator::Schedule(Time(xfsm->registers[XFSM_ARG2]*1000), 
                                &TcpSocketBase::OnTimerReceived, this, xfsm->registers[XFSM_ARG0]);
            break;
        }
        case XFSM_SETGENERICTIMER:  // ARGO:: timeout   //arg1:: param
        {
            Time schedTime = Time(xfsm->registers[XFSM_ARG0]*1000);
            fprintf(stderr, "*** Executing setGenericTimer at time %f, scheduled at time %f\n", 
                            Simulator::Now().GetSeconds(),
                            schedTime.GetSeconds());
            Simulator::Schedule(schedTime, &TcpSocketBase::OnGenericTimerReceived, this, 
                                0);
            break;
        }
        case XFSM_REMOVETIMER: // ARG0:: sequNumber
        {
          xfsm->removeTimer(xfsm->registers[XFSM_ARG0]);
          break;
        }
        case XFSM_SENDEMPTYPACKET: //XFSM_TCPFLAGS: store here the flags you want to be true for this packet
        {
          fprintf(stderr, "executing SendEmptyPacket with flags %d\n", xfsm->registers[XFSM_TCPFLAGS]);
          SendEmptyPacket(xfsm->registers[XFSM_TCPFLAGS]);
          break;
        }
        case XFSM_CLOSESOCKET:
        {
          CloseAndNotify();
          break;
        }
        default:
            break; // illegal action
    }
  }
}
...
TcpSocketBase::ProcessSynSent (Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
  ...
  // Extract the flags. PSH and URG are not honoured.
  uint8_t tcpflags = tcpHeader.GetFlags () & ~(TcpHeader::PSH | TcpHeader::URG);
  fprintf(stderr, "NextTxSequence ISSSSS %d   %d\n", (uint32_t)m_tcb->m_nextTxSequence.Get().GetValue(),(uint32_t) tcpHeader.GetAckNumber().GetValue());
  ...
  if (tcpflags == 0)
    ...
    { // Handshake completed
      ...
      // Here we're waking up our xfsm in case a SYNACK arrives
      ...
      if (xfsm_configured) {
        ...
        ts = DynamicCast<const TcpOptionTS> (tcpHeader.GetOption (TcpOption::TS));
        if (ts == 0) {
          if (tcpflags == (TcpHeader::ACK | TcpHeader::SYN))
            xfsm->xfsmWakeup(XFSM_SYNACK, seq.GetValue(), 0);
        }
        else {
          Time m = TcpOptionTS::ElapsedTimeFromTsValue (ts->GetEcho ());
          uint32_t packetRTT = (uint32_t) m.GetMicroSeconds();
          //uint32_t packetACK = (uint32_t) ackNumber.GetValue();
          if (tcpflags == (TcpHeader::ACK | TcpHeader::SYN))
            xfsm->xfsmWakeup(XFSM_SYNACK, seq.GetValue(), packetRTT);
        }
        if (tcpflags == (TcpHeader::ACK | TcpHeader::SYN)) {
          fprintf(stderr, "------ tableLookup()\n");
          TableEntry *match = xfsm->tableLookup();
      ...
      //Normal routine in case our xfsm is not set
      SendEmptyPacket (TcpHeader::ACK);
      SendPendingData (m_connected);
    }
      Simulator::ScheduleNow (&TcpSocketBase::ConnectionSucceeded, this);
  ...
  if (packet->GetSize () > 0 && !(tcpflags & TcpHeader::ACK))
    ...
    { // Got FIN, respond with ACK and move to next state
      ...
      if (xfsm_configured)  {
        syncXfsmRegisters();
        xfsm->setTcpFlags(tcpHeader);
        if (tcpflags == (TcpHeader::FIN | TcpHeader::ACK))
          xfsm->xfsmWakeup(XFSM_FINACK, 0, 0); //nothing to pass as parameters in this case
        else if (tcpflags == TcpHeader::FIN)
          xfsm->xfsmWakeup(XFSM_FIN, 0, 0);
        fprintf(stderr, "------ tableLookup()\n");
        TableEntry *match = xfsm->tableLookup();
        ...
        if (match != NULL){
          ...
          fprintf(stderr, "no matches wew found\n");
      }
...
TcpSocketBase::SendEmptyPacket (uint8_t flags)
{
  ...
  if (m_endPoint != 0)
    ...
    //TDB only in an xfsm context!
    if (hasFin && xfsm_configured) {
  ...
  if (m_closeOnEmpty && (remainingData == 0))
    {
      ...
      if (!xfsm_configured) {
          //This operation must be done only in a non-xfsm context!
          flags |= TcpHeader::FIN;
        if (m_state == ESTABLISHED)
          { // On active close: I am the first one to send FIN
            NS_LOG_DEBUG ("ESTABLISHED -> FIN_WAIT_1");
...
TcpSocketBase::BytesInFlight () const
{
  ...
  fprintf(stderr, "There are %u bytes in flight\n", bytesInFlight);
